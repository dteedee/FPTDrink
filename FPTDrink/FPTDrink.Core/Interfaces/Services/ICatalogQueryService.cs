...
namespace FPTDrink.Core.Interfaces.Services
{
	public interface ICatalogQueryService
	{
		...
		Task<(IReadOnlyList<Product> items, int total)> GetProductsAsync(int page, int pageSize, string? categoryId, string? supplierId, string? q, decimal? priceFrom, decimal? priceTo, string? sort, string? cartId, CancellationToken cancellationToken = default);
