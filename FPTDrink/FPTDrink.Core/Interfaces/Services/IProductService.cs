...
namespace FPTDrink.Core.Interfaces.Services
{
	...
	{
		Task<IReadOnlyList<Product>> GetListAsync(string status, string? search, CancellationToken cancellationToken = default);
		Task<Product?> GetByIdAsync(string id, CancellationToken cancellationToken = default);
		Task<Product> CreateAsync(Product model, CancellationToken cancellationToken = default);
		Task<bool> UpdateAsync(Product model, CancellationToken cancellationToken = default);
